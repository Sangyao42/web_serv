...
void	process::ProcessPostRequest(struct Client *clt)
{
	...
	enum file::FileManiError file_error = file::FileManiError::kFileManiNoError;
	//path is in the clt->path
	if (stat(clt->path.c_str(), &clt->stat_buff) == 0)  //file exists
	{
		if (S_ISDIR(clt->stat_buff.st_mode))
		{
			if (access(clt->path.c_str(), W_OK | X_OK) != 0)
			{
				clt->status_code = k403;
				return (res_builder::GenerateErrorResponse(clt));
			}
			if (IsDirFormat(clt->path) == false)
				clt->path = clt->path + "/";
			file_error = file::UploadFile(clt);
			if (file_error != file::FileManiError::kFileManiNoError)
			{
				if (file_error == file::FileManiError::kFileManiPermissionError)
					clt->status_code = k403;
				else
